...
public:
  FlyCamera();
  FlyCamera(glm::vec3 position, double fov) : position(position), fov(fov) {}
  ~FlyCamera() = default;
  ...
  void set_fov(double fov) { this->fov = fov; }
  ...
  glm::vec3 forward() const {
    const auto direction = glm::vec3(std::cos(glm::radians(this->yaw)),
